...
class cone:
    ...
    def normal(self, x, y, z):
        (x0, y0, z0, a, b, c, t) = self.p
        normal = np.array(
            [a * (a * (x - x0) + b * (y - y0) + c * (z - z0)) - (a ** 2 + b ** 2 + c ** 2) * np.cos(t) ** 2 * (x - x0), \
...
class AND:
    ...
    def f_rep(self, x, y, z):
        return self.fig1.f_rep(x, y, z) + self.fig2.f_rep(x, y, z) - np.sqrt(
            self.fig1.f_rep(x, y, z) ** 2 + self.fig2.f_rep(x, y, z) ** 2)

    # ∇AND = ∇f1 + ∇f2 - (f1∇f1+f2∇f2)/√f1^2+f2^2
    def normal(self, x, y, z):
        f1, f2, n1, n2 = self.fig1.f_rep(x, y, z), self.fig2.f_rep(x, y, z), self.fig1.normal(x, y,
                                                                                              z), self.fig2.normal(x, y,
                                                                                                                   z)
        # normal = n1+n2
        normal = np.array([n1[i] + n2[i] - (f1[i] * n1[i] + f2[i] * n2[i]) / np.sqrt(f1[i] ** 2 + f2[i] ** 2) for i in
                           range(len(f1))])
